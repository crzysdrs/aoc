...
fn adj(item: Item) -> impl Iterator<Item=Item> {    
    ...
    for (i,j) in &[(0,1), (1,0), (0,-1), (-1, 0)] {
        ...
        if y_offset == 2 && x_offset == 2 {
            match (j, i) {
                (0, 1) => {
                    adjs.extend((0..5).map(|x| Item { depth: depth - 1, pt: Point2 { x, y: 0}}));
                },
                (1, 0) => {
                    adjs.extend((0..5).map(|x| Item { depth: depth - 1, pt: Point2 { x : 0, y: x}}));
                }
                (-1, 0) => {
                    adjs.extend((0..5).map(|x| Item { depth: depth - 1, pt: Point2 { x : 4, y: x}}));
                }
                (0, -1) => {
                    adjs.extend((0..5).map(|x| Item { depth: depth - 1, pt: Point2 { x, y: 4}}));
                },
            ...
            if y_offset < 0 {
                //top edge
                adjs.push(Item { depth: depth + 1, pt: Point2 {x: 2, y: 1}});
            } else if y_offset >= width {
                //bottom edge
                adjs.push(Item { depth: depth + 1, pt: Point2 {x: 2, y: 3}});
            } else {
                y_on_grid = true;
            }
            ...
            if x_offset < 0 {
                //left edge
                adjs.push(Item { depth: depth + 1, pt: Point2 {x: 1, y: 2}});
            } else if x_offset >= width {
                //right edge
                adjs.push(Item { depth: depth + 1, pt: Point2 {x: 3, y: 2}});
            } else {
                x_on_grid = true;
            }

            if x_on_grid && y_on_grid {
                adjs.push(Item {depth, pt: Point2 {x: x_offset as u32, y: y_offset as u32}});
            }
        }
    }
    adjs.into_iter()   
    ...
    for d in depths {
        ...
        for y in 0..5 {
            for x in 0..5 {
                if y == 2 && x == 2 {
                    print!("?");
                } else  if items.contains(&Item {depth: d, pt: Point2 { x, y}}) {
                    print!("#");
                } else {
                    print!(".");
                }
...
// #....".to_string();
    ...
    for y in 0..width {
        for x in 0..width {
            if y == 2 && x == 2 {
                ...
                current.insert(Item {depth: 0, pt : Point2 { x: x as u32, y: y as u32 }});
            }
        }
    }
    ...
    for g in 0..200 {
        ...
        while let Some(item) = worklist.pop() {
            ...
            let neighbors : u32 =  adj(item).map(|x| if current.get(&x).is_some() { 1} else {0}).sum();
