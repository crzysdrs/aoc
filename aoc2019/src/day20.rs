...
//            U   P   P               ".to_string();
    ...
    println!("{} {}", max_x, max_y);
    let outer = move |p :&Point2<_>| {
        p.x < 4 || p.y < 4 || p.x > max_x - 4 || p.y > max_y - 4
    };
    ...
    fn recurse(outer : &dyn Fn(&Point2<i32>) -> bool, deps: &mut Graph<Tile,usize, petgraph::Undirected>, grid: &HashMap<Point2<i32>, char>,
               portal_edges : &[(Tile, Vec<Point2<i32>>)],
               all_nodes: &mut HashMap<(u32, Point2<i32>), NodeIndex<u32>>, depth: u32, max_depth: u32)
    {
        if depth > max_depth {
            return;
        }
        ...
        for (k,v) in portal_edges.iter() {                
            if v.len() == 2 {
                ...
                match (all_nodes.get(&(lower, outer[0])),
    ...
    recurse(&outer, &mut deps, &grid, &portal_edges, &mut all_nodes, 0, 256);
               
    let aa = portal_edges.iter().find_map(
        |(p, v)|
        if *p == Tile::Portal("AA".to_string()) {
