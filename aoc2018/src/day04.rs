...
pub fn p1() -> std::io::Result<()> {
    ...
    for s in f.lines() {
        ...
        let d = Utc.with_ymd_and_hms(year, mon, day, hour, min, 0).unwrap();
        v.push(Msg {
            time: d,
            content: c,
        });
    ...
    let m = hm.iter().max_by_key(|&(_g, (t, _v))| t).unwrap();
    let max_min = (m.1)
        .1
        .iter()
        .enumerate()
