...
fn convex_hull(pts : &[Point]) -> Vec<Point> {
    ...
    loop {
        ...
        for search in pts[1..].iter() {
            if endpoint == hull
                || orientation(&hull, &endpoint, &search) == Orient::CounterClockwise /* include collinear */ {
                    endpoint = *search;
                }
        }
        hull = endpoint;
...
pub fn p1() -> std::io::Result<()> {
    ...
    let mut f = std::io::BufReader::new(File::open("input/day6.txt")?);
    ...
    let mut next : Vec<Dist> = v.iter().map(|p| {
        //map.insert(*p, Some(*p));
        Dist { start: *p, next : *p }
    }
    ).collect();
    ...
    for dist in 1.. {
        ...
        for p in next.drain(..) {
            ...
            if updated {
                ...
                for x in &[-1,1] {
                    let next = Point { x: p.next.x + x, y: p.next.y };
                    next2.push(Dist {start : p.start, next})
                }
                for y in &[-1,1] {
                    let next = Point { x: p.next.x, y: p.next.y + y };
                    next2.push(Dist {start : p.start, next})
                }
            }
        }
        println!("Iteration {}, Next: {}", dist, next2.len());
        println!("{}/{}", next2.iter().filter(|p| exclude.contains(&p.start)).count(), next2.len());
    ...
    println!("{:?}", counts.iter().filter(|(k, v)| !exclude.contains(k)).collect::<Vec<_>>());
    println!("{:?}", counts.iter().filter(|(k, v)| !exclude.contains(k)).max_by_key(|&(k, v)| v).unwrap());
